...
def coarse_SfM_runner(
    ...
    if not triangulation_mode:
        # DetectorFreeSfM was released with an HLoc fork whose reconstruction
        # API accepted `prior_intrin_path` and `colmap_configs`.  Current HLoc
        # exposes the standard API instead.  This compatibility branch keeps
        # the original call when the fork is available and otherwise uses the
        # current HLoc/pycolmap camera-mode interface.
        try:
            reconstruction.main(
                Path(colmap_output_path),
                Path(temp_images_path),
                Path(pair_path),
                Path(feature_out),
                Path(match_out),
                Path(prior_intrin_path) if prior_intrin_path is not None else None,
                verbose=verbose,
                colmap_configs=colmap_configs,
            )
        except TypeError as error:
            if "colmap_configs" not in str(error):
                raise
            camera_mode = (
                pycolmap.CameraMode.SINGLE
                if colmap_configs and colmap_configs.get("ImageReader_single_camera", False)
                else pycolmap.CameraMode.AUTO
            )
            logger.warning(
                "Using current HLoc compatibility API; prior intrinsics are not "
                "passed to the standard reconstruction entry point."
            )
            reconstruction.main(
                Path(colmap_output_path),
                Path(temp_images_path),
