...
def get_keypoints(
    ...
    with h5py.File(str(path), "r", libver="latest") as hfile:
        # Upstream HLoc stores each image as a group with a `keypoints`
        # dataset. DetectorFreeSfM writes the image name directly as the
        # dataset. Support both layouts for compatibility.
        node = hfile[name]
        dset = node["keypoints"] if isinstance(node, h5py.Group) else node
        p = dset.__array__()
...
def find_pair(hfile: h5py.File, name0: str, name1: str):
    # DetectorFreeSfM stores direct datasets named "image0 image1".
    raw_pair = f"{name0} {name1}"
    if raw_pair in hfile:
        return raw_pair, False
    raw_pair = f"{name1} {name0}"
    if raw_pair in hfile:
        return raw_pair, True
...
def get_matches(path: Path, name0: str, name1: str) -> Tuple[np.ndarray]:
    with h5py.File(str(path), "r", libver="latest") as hfile:
        ...
        if isinstance(node, h5py.Dataset):
            matches = node.__array__().T
            if reverse:
                matches = np.flip(matches, -1)
            scores = np.ones(len(matches), dtype=np.float32)
            return matches, scores
        matches = node["matches0"].__array__()
        scores = node["matching_scores0"].__array__()
